...
# Split the data into training and testing sets based on time (60% train, 40% test)
...
study.optimize(objective, n_trials=5) #迭代调优次数修改

# 使用最佳参数重新构建模型进行测试数据集的训练和预测
...
best_model.fit(features_scaled[:split_index], target['Wretnd'][:split_index], epochs=study.best_trial.params['epochs'], batch_size=study.best_trial.params['batch_size'], verbose=1)

# 预测测试集并计算指标
test_predictions = best_model.predict(features_scaled[split_index:]).flatten()
mse = mean_squared_error(target['Wretnd'][split_index:], test_predictions)
rmse = np.sqrt(mse)
r2 = r2_score(target['Wretnd'][split_index:], test_predictions)
...
plt.plot(test_data['Date'], target['Wretnd'][split_index:], label='True Values')
